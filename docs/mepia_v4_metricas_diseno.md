...
6. **Nivel 3a y 3b cerrados, ambos 8/8.** Nivel 3a (N05, `tests/eval_test/eval_runner_n05.py`): 8/8 coinciden en decisión de escalar. Nivel 3b (Layer 3 completo N10→N14, `tests/eval_test/eval_runner_n3b.py`): 8/8 aprobados limpio, sin advertencias (ver decisiones #16 y #17). Sin probar todavía: la integración N05→N06 (confirmado en el código que hoy es un stub, "N06 aún no está implementado" — solo se loguea el intent, hueco real pendiente de decisión — no bloquea nada de lo ya probado, porque Nivel 3a/3b prueban N05 y Layer 3 en modo aislado, sin depender de que N06 exista).
